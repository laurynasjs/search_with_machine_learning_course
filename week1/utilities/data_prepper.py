...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        # hits = response['hits']['hits'] # there should only be one hit
        # print(response)
        # print("########")
        # print(response['hits']['hits'])
        # print(hits[0]['fields']['_ltrlog'][0]['log_entry'])
        # print("########")
        ...
        for hit in hits:
            ...
            # print(hit['_id'])
            # print(hit['_source']['sku'])
            features = hit['fields']['_ltrlog'][0]['log_entry']
            # print(features)
            
            for ftr in features:
                if ftr['name'] not in feature_results:
                    feature_results[ftr['name']] = []
                    
                feature_results[ftr['name']].append(ftr.get('value', 0))
